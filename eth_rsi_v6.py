...
def run_period(iv, name):
    ...
    if bu and pr>bu:
        ...
        if ac.can_push(k):
            title="⚠️ 涨太快了别追"
            body=("%s\n现价$%.0f，已经冲到通道外面了，有点过热。\n"
                  "👉 等回踩$%.0f再考虑多，现在追容易买在山尖。\n%s")%(hr,pr,bm,now_str)
    ...
    for p in RSI_PERIODS:
        if p in r and r[p]<LOW_THRESHOLD:
            ...
            if ac.can_push(k):
                extra=""
                if pr<bl: extra+=" 而且跌到通道外面了"
                elif ema_s and ema_l and pr<min(ema_s,ema_l): extra+=" 大趋势还是向下的"
                if vr>VOL_SURGE and pch<0: extra+=" 有人在放量砸盘"
                title="⚠️ 短线跌过头了"
                body=("%s\nRSI到了%.0f，属于超卖区域，别追空。\n"
    ...
    for p in RSI_PERIODS:
        if p in r and r[p]>HIGH_THRESHOLD:
            ...
            if ac.can_push(k):
                extra=""
                if pr>bu: extra+=" 而且冲到通道外面了"
                elif ema_s and ema_l and pr>max(ema_s,ema_l): extra+=" 大趋势还是向上的"
                if vr<VOL_SHRINK: extra+=" 但缩量上涨，小心假突破"
                title="⚠️ 短线涨过头了"
                body=("%s\nRSI到了%.0f，属于超买区域，别追多。\n"
                      "👉 等放量滞涨或回踩再考虑。%s\n%s")%(hr,r[p],extra,now_str)
                if push(title,body): pc+=1
    ...
    if v and len(v)>=22:
        if vr>VOL_SURGE and pch<-PRICE_CHG_MIN:
            ...
            if ac.can_push(k):
                extra=""
                if r[6]<30: extra="RSI也超卖了"
                title="🔴 有人在砸盘"
                body=("%s\n现价$%.0f，跌了%.2f%%，成交量大平时的%.1f倍。\n"
                      "👉 空头在发力，别接飞刀。%s\n%s")%(hr,pr,pch,vr,extra,now_str)
                if push(title,body): pc+=1
        ...
        if vr<VOL_SHRINK and pch>PRICE_CHG_MIN*0.6 and r[6]>65:
            ...
            if ac.can_push(k):
                title="🐻 涨不动了小心回调"
                body=("%s\n价格涨了但成交量只有平时的%.2f倍，量价背离。\n"
                      "👉 如果RSI拐头+突然放量，可能是个做空机会。\n%s")%(hr,vr,now_str)
                if push(title,body): pc+=1
        ...
        if vr>VOL_SURGE and abs(pch)<0.2:
            k="vtop_%s"%iv
            if ac.can_push(k):
                more="RSI在高位" if r[6]>65 else "多空还在博弈"
                title="⚖️ 多空在打架"
                body=("%s\n成交量放大但价格没动，%s。\n"
                      "👉 先观望，等方向选出来再说。\n%s")%(hr,more,now_str)
                if push(title,body): pc+=1
...
def main():
    print("="*60); print("ETH RSI监控 V6.0 - 人话版")
    ...
    print("量价+分析"); print("推送: Bark (人话版)"); print("="*60)
    while True:
        try:
